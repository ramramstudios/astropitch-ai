...
**On hold as of 2026-08-28: this branch will not merge into `master`, and the
checklist below will not be worked, until mobile mode on `master` is where we
want it.** The native shell wraps mobile mode as it exists today; locking that
in before mobile mode is settled risks redoing native-side work later, or
shipping a first release around a UI about to change under it. See
`outstanding.md` for the full note. Resume at item 1 below once mobile mode is
settled.
